...
for x in range(3):    
    ...
    ####RAINBOW####
    for x in range(1): #this is my favorite! Rainbow(0)
        rainbow(0) #speed
    print('RAinbow next for loop')
    for x in rainbow_range:
        rainbow(x * .01)
    clear_strip_color()

    #####COLOR CHASE####
    color_chase(red, random.choice(color_chase_range) * .5)  # Increase the number to slow down the color chase
    color_chase(yellow, random.choice(color_chase_range) * .5) 
    color_chase(green, random.choice(color_chase_range)* .5)
    for x in range(4): #this is my favorite! Rainbow(0)
        rainbow(0)
    color_chase(cyan, random.choice(color_chase_range)* .5) 
    color_chase(blue, random.choice(color_chase_range)* .5) 
    color_chase(purple, random.choice(color_chase_range)* .5) 
    clear_strip_color()
    ...
    ####COLOR CHASE LIST####
    color_chase_list(.03, purple_blue_list)
    color_chase_list(.02, greenyellow_list)
    color_chase_list(.01, bright_list)
    clear_strip_color()
    ...
    t_end = time.time() + 10 * 1  #run while loop for 60 seconds * x
    ...
    ####MULTIPLE SEND####
    for y in range(5):
        multiple_send(random.choice(color_chase_range), greenyellow_list, random.randint(1,60))# wait, color list, how many leds to send
        clear_strip_color()

    #RAINBOW COMET
    animations = [make_animation(strip) for strip in strips]
    group = AnimationGroup(*animations, )
    t_end = time.time() + 15 * 1  #run while loop for 60 seconds * x
    while time.time() < t_end:
        group.animate()
    
    #CUSTOM COLOR CHASE
    custom_color_chase_rainbow = CustomColorChase(pixels, speed=0.01, size=8, spacing=4)
    ...
    custom_color_chase_bgp = CustomColorChase(pixels, speed=0.01, colors=[BLUE, GREEN, PINK], size=5, spacing=2)
    ...
    for loop in range(3):
        ...
        for x in range(led_count):
            pixels[x] = color_choice
            pixels.show()
            time.sleep(.0001-(x*.0000001))   ## .01 works for ~30 leds .025 for ~10 leds
            pixels[x] = no_color
            pixels.show()
            #print(x)
            if x == led_count -15:
                for i in range(60):
                    ...
                    pixels[x+3] = no_color
                    pixels[x+4] = color_choice
                    pixels[x+5] = no_color
                    pixels[x+6] = color_choice
                    pixels[x+7] = no_color
                    pixels[x+8] = color_choice
                    pixels[x+9] = no_color
                    pixels[x+10] = color_choice
                    pixels[x+11] = no_color
                    pixels[x+12] = color_choice
                    pixels.show()
                    time.sleep(.05)
                    #inverse
                    pixels[x] = no_color
                    ...
                    pixels[x+2] = no_color
                    pixels[x+3] = color_choice
                    pixels[x+4] = no_color
                    pixels[x+5] = color_choice
                    pixels[x+6] = no_color
                    pixels[x+7] = color_choice
                    pixels[x+8] = no_color
                    pixels[x+9] = color_choice
                    pixels[x+10] = no_color
                    pixels[x+11] = color_choice
                    pixels[x+12] = no_color
                    pixels.show()
                    time.sleep(.05)
                    if i % 10 == 0:
